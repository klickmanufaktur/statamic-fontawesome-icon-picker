...
// Also emit the brand slugs as PHP. Unlike the JS datasets (FA Pro derived,
// git-ignored), this ships with the package so the migration command can tell
// brand icons from classic ones on a source-only Packagist install.
const brandPhp =
    '<?php\n\n// Brand icon slugs (FontAwesome "brands" family), regenerated by bin/build-icons.mjs.\n'
    + '// Used by the migration command to distinguish brand icons from classic ones.\n\nreturn [\n'
    + datasets.brands.map((icon) => "    '" + icon.n + "',").join('\n')
    + '\n];\n'
fs.writeFileSync(path.join(__dirname, '../src/brand-icons.php'), brandPhp)
console.log('src/brand-icons.php:', datasets.brands.length)
